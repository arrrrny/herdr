...
`shell_mode = "auto"` starts login shells on macOS so login-only PATH setup such as `/usr/libexec/path_helper` and Homebrew shell initialization runs in new panes. On other platforms, it keeps the existing non-login shell behavior. Use `"login"` to force login-shell startup or `"non_login"` to force non-login startup. Command panes and explicit argv launches keep their existing command execution paths.

Detached custom command keybindings (`type = "shell"`) and popup/pane custom command keybindings (`type = "popup"`, `type = "pane"`) spawn through the user's login shell with `-lc` on macOS and Linux so they inherit the same login-shell PATH setup as interactive panes (Homebrew's `~/.zprofile` / `~/.bash_profile`, `/usr/libexec/path_helper`, etc.). This is what makes bindings like `command = "herdr agent focus atlas"` work when Herdr is launched under `brew services` / launchd with a stripped process PATH. Spawn and exec failures (e.g. command not found in the resolved PATH) are now surfaced line-by-line in the Herdr server log instead of being silently discarded.
